...
function EvaluationContent() {
  ...
  if (data.evaluation_score === null) return <div className="page-shell py-24"><p className="eyebrow">Draft diagnostics</p><h1 className="balanced mt-5 max-w-3xl font-display text-5xl font-medium tracking-[-0.05em] sm:text-7xl">Inspect this draft.</h1><p className="mt-6 max-w-2xl text-muted-foreground">Check the current draft against measured voice targets, editing constraints, platform limits, and retrieved evidence. These automatic diagnostics do not establish authentic voice or pass the assignment&apos;s human review.</p><Button className="mt-8" onClick={() => evaluation.mutate()} size="lg">Run diagnostics</Button><ReviewGate /></div>;
