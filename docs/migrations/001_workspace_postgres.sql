-- Initial workspace schema, PostgreSQL. Prefer the version-checking admin CLI.
-- Apply once to the explicitly configured workspace database as its migration owner.
BEGIN;
...
DROP TRIGGER IF EXISTS cv_workflow_revisions_immutable ON cv_workflow_revisions;

CREATE TRIGGER cv_workflow_revisions_immutable BEFORE UPDATE OR DELETE ON cv_workflow_revisions
                FOR EACH ROW EXECUTE FUNCTION cv_reject_history_mutation();

DROP TRIGGER IF EXISTS cv_workflow_reviews_immutable ON cv_workflow_reviews;

CREATE TRIGGER cv_workflow_reviews_immutable BEFORE UPDATE OR DELETE ON cv_workflow_reviews
                FOR EACH ROW EXECUTE FUNCTION cv_reject_history_mutation();

DROP TRIGGER IF EXISTS cv_review_runs_immutable ON cv_review_runs;

CREATE TRIGGER cv_review_runs_immutable BEFORE UPDATE OR DELETE ON cv_review_runs
                FOR EACH ROW EXECUTE FUNCTION cv_reject_history_mutation();

INSERT INTO cv_schema_migrations(version) VALUES (1) ON CONFLICT(version) DO NOTHING;
